...
export default function FormPage({hash, image, title, text, form, imageStyle, navigationPage, hashList }) {
  ...
  return (
    <div className="flex items-start md:justify-around md:mt-12">
      <div className="flex flex-col items-center mb-6 md:mb-0">
        ...
        <a onClick={() => navigateToNext("/uploadmodel")}><p className="text-lg font-poppins text-white text-gray/70 max-w-[340px]">{!hash ? text : "Secured by Lighthouse"}</p></a>
        {hash?<>
        <div className="mt-5 text-white font-semibold data-card-2 rounded-lg p-2">{hash}</div>
        <a onClick={navigateToNext}><div className="flex items-center mt-5 text-white  font-semibold data-card-3 rounded-lg p-2 px-3">
