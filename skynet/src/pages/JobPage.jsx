...
const WORKERS = [
  "0x8c42E233514011BfcFdF3e0c1258AfaDdAC5C191",
  "0xd46c37532b3473834cDB6DdBF48287B8E179e4a4",
];
const EPOCHS = 1;

function JobPage() {
  const [trainedWeights, setTrainedWeights] = useState("");
  const [epochPkts, setEpochPkts] = useState([]);
  ...
  const reducer = (state, action) => {
    ...
    switch (action.type) {
      case "ADD_MESSAGE":
        ...
        if (trainedWeights) {
          const n = WORKERS.indexOf(action.message.srcId) + 1;
          console.log(`Got trained weights! - ${n}`);
          invoke("store_n", { weights: trainedWeights, number: n });
          // invoke("store_weights", { weights });
          // console.log("Getting trained weights ...");
          // invoke("get_weights").then((weights) => {
          //   console.log(weights);
      ...
      case "SEND_MESSAGE":
        const SYN = action.SYN ?? state.sendCounter + 1;
        ...
        if (action.body.weights) {
          setEpochPkts((epochPkts) => [...epochPkts, SYN]);
        }
